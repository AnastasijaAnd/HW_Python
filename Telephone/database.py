...
def write_name(name):
    with open("telphon.txt", "a", encoding="UTF-8") as file:
        file.write(name)

def search_smth_in_phone_book():
    number = input_search_option()
    look_world = input_want_to_find()
    with open("telphon.txt", "r", encoding="UTF-8") as file:
        lst = file.readlines()
        fnd_lst = []
        if number == 1:
            for row in lst:
...
def change_smth_in_phone_book():
    ...
    with open("telephon.txt", "w", encoding="UTF-8") as file:
        for row in lst:
            file.write(row)

def delete_smth_in_phone_book():
    with open("telephon.txt", "r", encoding="UTF-8") as file:
        lst = file.readlines()
